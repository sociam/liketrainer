...
# merge everything, select only male workers
...
male_only = demog_comments_annotations[demog_comments_annotations.gender == 'male']

# remove unwanted columns

for column in ['year', 'logged_in', 'ns', 'sample', 'toxicity', 'english_first_language', 'education']:
	del male_only[column]

# create a new column with toxicity score, i.e. the mean average toxicity rating for that comment

male_only['toxicity_average'] = male_only['toxicity_score'].groupby(male_only['rev_id']).transform(np.mean)

# create a new column with a toxicity classification, which is true if the toxixity score < 0

male_only['toxicity_class'] = male_only['toxicity_average'] < 0.00

# remove newlines and tabs

male_only['comment'] = male_only['comment'].apply(lambda x: x.replace("NEWLINE_TOKEN", " "))
male_only['comment'] = male_only['comment'].apply(lambda x: x.replace("TAB_TOKEN", " "))

print male_only

train_male_only = male_only.query("split=='train'")
test_male_only = male_only.query("split=='test'")
...
clf = clf.fit(train_male_only['comment'], train_male_only['toxicity_class'])

auc = roc_auc_score(test_male_only['toxicity_class'], clf.predict_proba(test_male_only['comment'])[:, 1])
