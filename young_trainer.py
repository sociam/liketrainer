...
# merge everything, select only young workers
...
young_only = demog_comments_annotations[demog_comments_annotations.age_group == '30-45']

# remove unwanted columns

for column in ['year', 'logged_in', 'ns', 'sample', 'toxicity', 'english_first_language', 'education']:
	del young_only[column]

# create a new column with toxicity score, i.e. the mean average toxicity rating for that comment

young_only['toxicity_average'] = young_only['toxicity_score'].groupby(young_only['rev_id']).transform(np.mean)

# create a new column with a toxicity classification, which is true if the toxixity score < 0

young_only['toxicity_class'] = young_only['toxicity_average'] < 0.00

# remove newlines and tabs

young_only['comment'] = young_only['comment'].apply(lambda x: x.replace("NEWLINE_TOKEN", " "))
young_only['comment'] = young_only['comment'].apply(lambda x: x.replace("TAB_TOKEN", " "))

print young_only

train_young_only = young_only.query("split=='train'")
test_young_only = young_only.query("split=='test'")
...
clf = clf.fit(train_young_only['comment'], train_young_only['toxicity_class'])

auc = roc_auc_score(test_young_only['toxicity_class'], clf.predict_proba(test_young_only['comment'])[:, 1])
